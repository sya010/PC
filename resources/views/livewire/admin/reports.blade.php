<div class="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 dark:text-dark-300 mt-1">
                View financial reports, order statistics, stock audits, and selling performance.
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white dark:bg-dark-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-dark-800 flex items-center gap-5 hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Total Revenue</span>
                <span class="text-2xl font-black text-gray-900 dark:text-dark-100">{{ number_format($totalRevenue, 0) }} IQD</span>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-dark-800 flex items-center gap-5 hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Estimated Profit (25%)</span>
                <span class="text-2xl font-black text-emerald-600 dark:text-emerald-400">{{ number_format($estimatedProfit, 0) }} IQD</span>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-dark-800 flex items-center gap-5 hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Active Deliveries</span>
                <span class="text-2xl font-black text-amber-600 dark:text-amber-400">{{ $activeDeliveries }} Orders</span>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-dark-800 flex items-center gap-5 hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div>
                <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Out of Stock Items</span>
                <span class="text-2xl font-black text-rose-600 dark:text-rose-400">{{ $outOfStockCount }} Products</span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 bg-white dark:bg-dark-900 rounded-2xl shadow-sm border border-gray-100 dark:border-dark-800 overflow-hidden">
            <div class="p-6 border-b border-gray-100 dark:border-dark-800">
                <h3 class="text-lg font-bold text-gray-900 dark:text-dark-100">Top Selling Products</h3>
                <p class="text-xs text-gray-400 mt-1">Ranking of items by total quantity sold.</p>
            </div>
                ...
                <table class="w-full text-{{ in_array(app()->getLocale(), ['ar', 'ku']) ? 'right' : 'left' }} border-collapse text-sm">
                    <thead>
                        <tr class="bg-gray-50/50 dark:bg-dark-950 border-b border-gray-100 dark:border-dark-800 text-xs font-bold text-gray-500 dark:text-dark-400 uppercase tracking-wider">
                            ...
                            <th class="px-6 py-4 text-center">Quantity Sold</th>
                            <th class="px-6 py-4">Total Revenue</th>
                            <th class="px-6 py-4 text-center">Remaining Stock</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-dark-800 font-medium text-gray-700 dark:text-dark-200">
                        @forelse($topSellingItems as $item)
                            <tr class="hover:bg-gray-50/30 dark:hover:bg-dark-900/30 transition-colors">
                                ...
                                <td class="px-6 py-4 text-brand-base dark:text-brand-light font-bold">{{ number_format($item->total_sales, 0) }} IQD</td>
