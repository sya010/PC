    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-4 border-b border-gray-100 pb-2">Account Details</h3>
                
                <div class="space-y-4">
                    <div>
                        <span class="block text-xs font-semibold text-gray-400 uppercase tracking-wide">User ID</span>
                        <span class="text-gray-900 font-medium">#{{ $user->id }}</span>
                    </div>
                    <div>
                        <span class="block text-xs font-semibold text-gray-400 uppercase tracking-wide">Joined On</span>
                        <span class="text-gray-900 font-medium">{{ $user->created_at->format('M d, Y, h:i A') }}</span>
                    </div>
                    <div>
                        <span class="block text-xs font-semibold text-gray-400 uppercase tracking-wide">Last Updated</span>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    @if(!$user->is_admin)
                        <span class="text-xs text-orange-500 bg-orange-50 px-2 py-1 rounded font-medium">This user is not an admin, so no admin actions are logged.</span>
                    @endif
                </div>
                ...
                @if($user->is_admin)
                    <div class="flow-root">
                        <ul role="list" class="-mb-8">
                            ...
                            @empty
                                <li class="text-center py-4 text-gray-500 text-sm">No activity recorded for this admin yet.</li>
                            @endforelse
                        </ul>
                    </div>
                @else
                    <div class="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                        <p class="text-gray-500">User is not an admin, so no task logs are available.</p>
                    </div>
                @endif
            </div>
        </div>
