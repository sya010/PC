<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 bg-clip-text text-transparent bg-gradient-to-r from-brand-base to-brand-light">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-5">
                        <div class="group">
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5 group-focus-within:text-brand-base transition-colors">{{ __('messages.admin.product_form.product_name') }}</label>
                            <input type="text" wire:model="name" maxlength="255" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50/50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-brand-base/10 focus:border-brand-base focus:bg-white transition-all duration-200 hover:border-brand-base/30 shadow-sm" placeholder="{{ __('messages.admin.product_form.product_name_placeholder') }}">
                            @error('name') <span class="text-red-500 text-sm font-medium mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <div class="group">
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5 group-focus-within:text-brand-base transition-colors">{{ __('messages.admin.product_form.description') }}</label>
                            <textarea wire:model="description" maxlength="5000" rows="4" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50/50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-brand-base/10 focus:border-brand-base focus:bg-white transition-all duration-200 hover:border-brand-base/30 shadow-sm resize-none" placeholder="{{ __('messages.admin.product_form.description_placeholder') }}"></textarea>
                            @error('description') <span class="text-red-500 text-sm font-medium mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <div class="grid grid-cols-2 gap-5">
                            <div class="group">
                                <label class="block text-sm font-semibold text-gray-700 mb-1.5 group-focus-within:text-brand-base transition-colors">{{ __('messages.admin.product_form.price') }}</label>
                            ...
                            <div class="group">
                                <label class="block text-sm font-semibold text-gray-700 mb-1.5 group-focus-within:text-brand-base transition-colors">{{ __('messages.admin.product_form.stock_quantity') }}</label>
                                <input type="number" wire:model="stock" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50/50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-brand-accent/10 focus:border-brand-accent focus:bg-white transition-all duration-200 hover:border-brand-base/30 shadow-sm">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button type="button" wire:click="addSpec" class="text-sm px-4 py-2 bg-brand-base/10 text-brand-base rounded-lg hover:bg-brand-base/20 font-bold transition-all hover:shadow-sm">
                    ...
                    <div class="space-y-4">
                        @foreach($specs as $index => $spec)
                            <div class="flex gap-4 group">
                                <input type="text" wire:model="specs.{{ $index }}.key" maxlength="100" placeholder="{{ __('messages.admin.product_form.key_placeholder') }}" class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50/50 text-sm focus:outline-none focus:ring-4 focus:ring-brand-accent/10 focus:border-brand-accent focus:bg-white transition-all shadow-sm">
                                <input type="text" wire:model="specs.{{ $index }}.value" maxlength="255" placeholder="{{ __('messages.admin.product_form.value_placeholder') }}" class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50/50 text-sm focus:outline-none focus:ring-4 focus:ring-brand-accent/10 focus:border-brand-accent focus:bg-white transition-all shadow-sm">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-5">
                        <div class="group" x-data="{ 
                            ...
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5 group-focus-within:text-brand-base transition-colors">{{ __('messages.admin.products.category') }}</label>
                            ...
                            <div class="flex gap-2 h-[48px]">
                                ...
                                <div class="relative flex-1" x-show="!isAdding">
                                    <button type="button" @click="open = !open; isAdding = false" @click.away="open = false" 
                                        class="w-full h-full px-4 rounded-xl border border-gray-200 bg-gray-50/50 text-{{ in_array(app()->getLocale(), ['ar', 'ku']) ? 'right' : 'left' }} text-gray-900 cursor-pointer focus:outline-none focus:ring-4 focus:ring-brand-accent/10 focus:border-brand-accent focus:bg-white transition-all duration-200 flex items-center justify-between shadow-sm">
                                        ...
                                        <div class="max-h-60 overflow-y-auto py-1">
                                            <template x-for="option in options" :key="option">
                                                <div @click="selected = option; open = false" 
                                                    class="px-4 py-2.5 hover:bg-brand-base/5 cursor-pointer flex items-center justify-between group/item transition-colors">
                                                    <span x-text="option" class="font-medium text-gray-700 group-hover/item:text-brand-accent"></span>
                                ...
                                <input x-show="isAdding" type="text" x-model="newCategory" @keydown.enter.prevent="addCategory()" placeholder="{{ __('messages.admin.product_form.add_new_category') }}" 
                                    class="flex-1 w-full h-full px-4 rounded-xl border border-gray-200 focus:outline-none focus:ring-4 focus:ring-brand-accent/10 focus:border-brand-accent bg-white shadow-sm transition-all text-sm" style="display: none;">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-4">
                         ...
                         <div
                            ...
                            :class="isDropping ? 'border-brand-base bg-brand-base/5' : 'border-gray-200 hover:border-brand-light hover:bg-gray-50'"
                        >
                        ...
                        <div x-data="{ show: false }">
                             ...
                             <div x-show="show" class="group">
                                <input type="text" wire:model="existingImage" placeholder="https://..." class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-gray-50/50 text-xs text-gray-700 focus:outline-none focus:ring-2 focus:ring-brand-base/10 focus:border-brand-base focus:bg-white transition-all">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-3">
                    <button type="submit" class="w-full py-3.5 px-4 bg-gradient-to-r from-brand-accent to-brand-base text-white font-bold rounded-xl shadow-lg shadow-brand-base/20 hover:shadow-brand-base/30 hover:opacity-90 transform hover:-translate-y-0.5 transition-all flex justify-center items-center gap-2">
