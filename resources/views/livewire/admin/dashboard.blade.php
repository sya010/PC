<div>
    <div class="mb-8">
        ...
        <p class="text-content-secondary dark:text-dark-200 mt-1">{{ __('messages.admin.dashboard.welcome') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-surface-primary dark:bg-dark-800 p-6 rounded-2xl border border-border-subtle dark:border-dark-700 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
            ...
            <div class="relative">
                <p class="text-sm text-content-secondary dark:text-dark-200 font-bold uppercase tracking-wider">{{ __('messages.admin.dashboard.total_sales') }}</p>
                <h3 class="text-3xl font-bold text-content-primary dark:text-dark-100 mt-2">{{ number_format($totalSales, 0) }} {{ __('messages.currency') }}</h3>
        ...
        <div class="bg-surface-primary dark:bg-dark-800 p-6 rounded-2xl border border-border-subtle dark:border-dark-700 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
            ...
            <div class="relative">
                <p class="text-sm text-content-secondary dark:text-dark-200 font-bold uppercase tracking-wider">{{ __('messages.admin.dashboard.active_orders') }}</p>
        ...
        <div class="bg-surface-primary dark:bg-dark-800 p-6 rounded-2xl border border-border-subtle dark:border-dark-700 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
            ...
            <div class="relative">
                <p class="text-sm text-content-secondary dark:text-dark-200 font-bold uppercase tracking-wider">{{ __('messages.admin.dashboard.total_products') }}</p>
        ...
        <div class="bg-surface-primary dark:bg-dark-800 p-6 rounded-2xl border border-border-subtle dark:border-dark-700 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
            <div class="absolute right-0 top-0 w-32 h-32 bg-brand-base/5 rounded-bl-full -mr-8 -mt-8 transition-transform group-hover:scale-110"></div>
            <div class="relative">
                <p class="text-sm text-content-secondary dark:text-dark-200 font-bold uppercase tracking-wider">{{ __('messages.admin.dashboard.new_users') }}</p>
    ...
    <div class="bg-surface-primary dark:bg-dark-800 rounded-2xl shadow-sm border border-border-subtle dark:border-dark-700 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-{{ in_array(app()->getLocale(), ['ar', 'ku']) ? 'right' : 'left' }} text-sm text-content-secondary dark:text-dark-200">
                <thead class="bg-surface-secondary dark:bg-dark-900 text-xs uppercase font-bold text-content-muted dark:text-dark-300 tracking-wider">
                    <tr>
                        ...
                        <tr class="hover:bg-surface-secondary dark:hover:bg-dark-900 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    ...
                                    $color = $colors[$order->status] ?? 'bg-surface-secondary dark:bg-dark-700 text-content-secondary dark:text-dark-200 border-border-subtle dark:border-dark-600';
                                    $statusKeys = [
                            ...
                            <td class="px-6 py-4 text-content-secondary dark:text-dark-200">{{ $order->created_at->format('M d, Y') }}</td>
                            <td class="px-6 py-4 text-{{ in_array(app()->getLocale(), ['ar', 'ku']) ? 'left' : 'right' }} font-bold text-content-primary dark:text-dark-100">{{ number_format($order->total_amount, 0) }} {{ __('messages.currency') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-content-muted dark:text-dark-300 font-medium">{{ __('messages.admin.dashboard.no_recent_orders') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
