<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                @foreach($cartItems as $id => $item)
                    <div class="neu-card p-4 flex flex-col sm:flex-row gap-4 items-center">
                        ...
                        <div class="flex-1 text-center sm:text-left rtl:sm:text-right">
                            ...
                            <p class="text-sm text-[var(--color-text-secondary)] capitalize">{{ $item['category'] ?? 'Product' }}</p>
                            <p class="font-bold text-indigo-600 mt-1">{{ number_format($item['price'], 0) }} IQD</p>
                        </div>
                ...
                <div class="neu-card p-6 sticky top-24">
                    ...
                    <div class="space-y-4 mb-6">
                        <div class="flex justify-between text-[var(--color-text-secondary)]">
                            <span>Subtotal</span>
                            <span>{{ number_format($total + $discount, 0) }} IQD</span>
                        </div>
                        @if($discount > 0)
                            <div class="flex justify-between text-green-600 font-medium">
                                <span>Discount</span>
                                <span>-{{ number_format($discount, 0) }} IQD</span>
                            </div>
                        @endif
                        <div class="flex flex-col gap-2 pt-2 pb-2">
                             <div class="flex gap-2">
                                <input type="text" wire:model="couponCode" placeholder="Promo Code" class="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500">
                                <button wire:click="applyCoupon" class="px-3 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-800 transition-colors">Apply</button>
                             </div>
                        </div>
                        <div class="flex justify-between text-[var(--color-text-secondary)]">
                            <span>Shipping</span>
    ...
    @else
        <div class="text-center py-20 bg-[var(--color-bg-secondary)] rounded-3xl">
            ...
            <h2 class="text-2xl font-bold text-[var(--color-text-primary)] mb-2">Your cart is empty</h2>
            <p class="text-[var(--color-text-secondary)] mb-8">Looks like you haven't added any products yet.</p>
